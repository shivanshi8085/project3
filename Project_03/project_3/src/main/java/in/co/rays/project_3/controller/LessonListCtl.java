...
public class LessonListCtl extends BaseCtl {
	...
	protected void preload(HttpServletRequest request) {
		LessonModelInt bmodel = ModelFactory.getInstance().getLessonModel();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		try {
			...
			ArrayList a = (ArrayList<LessonDTO>) list;
		...
		try {
			...
			if (OP_SEARCH.equalsIgnoreCase(op) || "Next".equalsIgnoreCase(op) || "Previous".equalsIgnoreCase(op)) {
				...
				if (ids != null && ids.length > 0) {
					LessonDTO deletedto = new LessonDTO();
					for (String id : ids) {
			...
			dto = (LessonDTO) populateDTO(request);
			list = model.search(dto, pageNo, pageSize);
